...
@app.get('/video_feed_emotion')
async def video_feed_emotion():
    # Video streaming route. Put this in the src attribute of an img tag
    return StreamingResponse(stream.gen_frames('emotion'), media_type="multipart/x-mixed-replace; boundary=frame")
...
async def home_page(request: Request):
    return templates.TemplateResponse("object_detection.html", {"request": request, "dtype": "emotion"})
